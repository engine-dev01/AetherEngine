...
| FirebaseInitProvider (guest provider — GATED) | 2026-09-15 | ตัวจุดชนวน `m7.*` GMS measurement dynamite บน main-looper → SE 'Unknown calling package name' → process จบที่ dispatch (Zygote 'exited cleanly (0)' ไม่มี FATAL/ANR — handler เป็น observer ไม่ใช่ lifeline) | device proof apk 0955c54 (P4 round): 22:12:35.933 FirebaseInitProvider onCreate → 22:12:41 exit; 09-14 00:31 build ที่ skip ทั้ง family ไปลึกสุด (vdex ×4) |
